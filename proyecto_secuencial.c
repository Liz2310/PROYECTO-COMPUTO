...
int img_to_txt(double *img, int x, int y, int device);

int main() {

    struct Image *img = ( struct Image*) malloc(sizeof(struct Image));
    if (!getImage("./imgs/1/", img)) return 1;

    examineImg(img);

    int len_array = img->x*img->y;
    double *d_R, *d_G, *d_B, *d_L; //Device en GPU
    double *h_L = (double*) malloc(sizeof(double) * len_array);

    clock_t begin = clock();

    for (int i = 0; i < len_array; i++) {
        h_L[i] = (img->red[i] * 0.21) + (img->green[i] * 0.72) + (img->blue[i] * 0.07);
    }
    ...
    printf("\nEl tiempo transcurrido en el CPU fue de %f segundos", time_spent);

    img_to_txt(h_L, img->x, img->y, 1);
...
int img_to_txt(double *img, int x, int y, int device){
    ...
    if(device == 0){
        filename = "./imgs/1/output_cpu.txt";
    } else {
        if(device == 1){
            filename = "./imgs/1/output_gpu.txt";
        }
    }
